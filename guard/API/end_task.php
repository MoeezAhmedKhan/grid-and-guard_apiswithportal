...
if($_POST['token'] == 'as23rlkjadsnlkcj23qkjnfsDKJcnzdfb3353ads54vd3favaeveavgbqaerbVEWDSC')
{
    ...
    $update = "UPDATE `accept_request` SET `status`= 'completed',`end_job_date`= '$current_date'  WHERE `user_id`= $user_id and `guard_id`= $guard_id and `task_id`= $task_id";
    $execute_update = mysqli_query($conn,$update);
                
    if($execute_update)
    {
        $on_demand = "UPDATE `on_demand_request` SET `status`= 'completed' , `end_location` = '$guard_location' WHERE `id`= $task_id";
        ...
        if($execute_on_demand)
        {
            ...
            $message = "Your account has been debited for $".$Fees." as job has been compelted.";
            $insert_notification = "INSERT INTO `notification`(`user_id`, `content`, `purpose`) VALUES ($user_id, '$message' , 'Payment')";
            $exec_insert = mysqli_multi_query($conn,$insert_notification);
