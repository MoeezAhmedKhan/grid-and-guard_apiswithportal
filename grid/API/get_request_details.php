...
if($_POST['token'] == 'as23rlkjadsnlkcj23qkjnfsDKJcnzdfb3353ads54vd3favaeveavgbqaerbVEWDSC')
{
    ...
    for($i = 0; $i < count($months); $i++)
    {
        ...
        $petroling_service = "SELECT p.id,p.user_id,p.start_date,p.end_date,p.service_address,p.week_days,p.petrols_daily,p.location,p.time,p.post_order,p.status,p.created_at,u.business_name,u.company_name,u.first_name,u.last_name FROM add_petroling_service p INNER JOIN user u on u.id = p.user_id WHERE p.user_id = $user_id";
        ...
        if($petrol_row > 0)
        {
         ...
         $standing_security = "SELECT s.`id`, s.`user_id`, s.`week_days`, s.`start_date`, s.`end_date`, s.`service_address`, s.`petrols_daily`, s.`shift_start_time`, s.`specific_post_order`, s.`location`, s.`time`, s.`post_orders`,s.status, s.`created_at`, u.business_name, u.company_name, u.first_name, u.last_name FROM `add_standing_security` s INNER JOIN user u on u.id = s.user_id WHERE s.user_id = $user_id";
         ...
         $on_demand_request = "SELECT o.`id`, o.`user_id`, o.`name_of_location`,o.`location`, o.`additional_notes`, o.`image`, o.`video`,o.status, o.`created_at`,u.business_name,u.company_name,u.first_name,u.last_name FROM `on_demand_request` o INNER JOIN user u on u.id = o.user_id WHERE o.user_id = $user_id";
