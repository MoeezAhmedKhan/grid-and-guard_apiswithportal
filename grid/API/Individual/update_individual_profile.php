...
if($_POST['token'] == 'as23rlkjadsnlkcj23qkjnfsDKJcnzdfb3353ads54vd3favaeveavgbqaerbVEWDSC')
{
    ...
    $filewithnewname = date("Ymdis")."Individual_Profile.".$imageFileType;
    ...
    $check_if_dataisin_db="SELECT * FROM `user` WHERE id = $u_id and role_id = 1";
    ...
    if(mysqli_num_rows($execute) > 0)
    {
                ...
                if($profile_size <= 5000000) /*5 mb*/
                {
                    if(strtolower($profile_type) == "image/jpg" || strtolower($profile_type) == "image/jpeg" || strtolower($profile_type) == "image/png" || strtolower($profile_type) == "image/jfif")
                    {
                        if($curent_img == "")
                        {
                            ...
                            $insert = "UPDATE `user` SET `first_name`= '$firstname',`last_name`= '$lastname',`email`= '$email',`username`= '$username',`phone`= '$phone',
                            `address`= '$address',`zipcode`= '$zipcode',`country`= '$country',`profile`= '$filewithnewname' WHERE id = $u_id";
                            ...
                            if($run_insert)
                            {
                                ...
                                [
                                    "status"=>true,
                                    "response_code"=>200,
                                    "message"=>"user account has been updated sucessfully.",
                                    "data"=>$temp
                                ];
                        ...
                        elseif($curent_img != "")
                        {
                            ...
                            $update = "UPDATE `user` SET `first_name`= '$firstname',`last_name`= '$lastname',`email`= '$email',`username`= '$username',`phone`= '$phone',
                            `address`= '$address',`zipcode`= '$zipcode',`country`= '$country',`profile`= '$filewithnewname' WHERE id = $u_id";
        ...
        elseif(!isset($_FILES['profile']))
        {
                ...
                $insert = "UPDATE `user` SET `first_name`= '$firstname',`last_name`= '$lastname',`email`= '$email',`username`= '$username',`phone`= '$phone',
                `address`= '$address',`zipcode`= '$zipcode',`country`= '$country' WHERE id = $u_id";
                $run_insert = mysqli_query($conn,$insert);
                if($run_insert)
                {
                    $temp = 
